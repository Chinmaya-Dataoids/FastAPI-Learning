...
#from fastapi.responses import JSONResponse # will use default execption handlers 
...
def update_post_partial(post_id: int, post_data: PostUpdate, db: Annotated[Session, Depends(get_db)]):
    ...
    #We only want to update fields that client send and not fields that they send empty
...
#for delete we usually return 204 response, which means request succeeded but we have no response body
